...
def rf(prompts: list, completions: list, **kwargs) -> list:
    rewards = []
    print("len_comp:------>",len(completions))
    print("len_prompt:------>",len(prompts))
    print("Prompt:------>",prompts)
    print("Completions:",completions)
    for prompt, completion in zip(prompts, completions):
        if not completion.strip():  # Reject empty completions
            rewards.append(-1.0)   # Strong penalty
            continue
        
        gt_data = gt_lookup.get(prompt)
        if not gt_data:
            rewards.append(0.01)   # Default for unseen prompts
            continue
        ...
        print("Embedding norms:", [np.linalg.norm(e) for e in embeddings])
        similarity = cosine_similarity([embeddings[0]], [embeddings[1]])[0][0]
        rewards.append(float(similarity))
    
    rewards = np.clip(rewards, -1.0, 1.0)  # Force rewards into [-1, 1]
    print(f"Reward batch sample - Prompt: {prompts[0][:30]}... | Completion: {completions[0][:100]}... | Reward: {rewards[0]:.2f}")
    print("Answer: -->",gt_data['answer'])
    return rewards
...
trainer = GRPOTrainer(
    ...
    args=training_args,
    train_dataset=data,
    report_to=["wandb"],  # or "tensorboard", or ["csv", "json"]
